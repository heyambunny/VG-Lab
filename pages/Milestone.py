...
st.set_page_config(
    page_title="Milestone Scheme",
    layout="wide")

kog = 'assets/KOG.jpeg'
cochin = 'assets/cochin.jpeg'
panIndia = 'assets/PAN_India.jpeg'

protect_page("Milestone")
if not st.session_state.get("authenticated"):
    st.switch_page("pages/Login.py")

st.title("IMP Milestone Scheme")
st.write("Scheme Duration: 01 Oct 2025 to 31 Mar 2025")
c1, c2, c3 = st.columns([1,1,1])
@st.dialog("Scheme Preview")
def show_preview(img, title):
    st.subheader(title)
    st.image(img, use_container_width=True)

with c1:
    if st.button("View KOG Scheme"):
        show_preview(kog, "KOG")

with c2:
    if st.button("View Cochin Scheme"):
        show_preview(cochin, "Cochin")

with c3:
    if st.button("View PAN India Scheme"):
        show_preview(panIndia, "Pan India")

st.divider()
p1, p2, p3 = st.columns([1,1,1])
file_1 = p1.file_uploader('Upload Oct Month File', type=['xls', 'csv'])
file_2 = p2.file_uploader('Upload Nov Month File', type=['xls', 'csv'])
file_3 = p3.file_uploader('Upload Dec Month File', type=['xls', 'csv'])
today = date.today()

if today.month == 1:
    file_4 = st.file_uploader('Upload Jan Month File', type=['xls', 'csv'])
elif today.month == 2: st.write("")
...
if st.button("Generate"):
    ...
    st.divider()
    summary = qualified_df.groupby(['Gift Qualification'])['Rishta Id'].count()
